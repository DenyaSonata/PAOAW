...
Scene_Controls.prototype.createControlsWindow = function() {
    ...
    this._controlsWindow = new Window_MenuCommand(rect);
    this._controlsWindow.setHandler('cancel', this.popScene.bind(this));
    this._controlsWindow.setHandler('ok', this.popScene.bind(this));
    this.addWindow(this._controlsWindow);
    this._controlsWindow.activate();
};

Scene_Controls.prototype.createControlsImage = function() {
    const bitmap = ImageManager.loadPicture('TutorialPage');
    bitmap.addLoadListener(() => {
        this._controlsSprite = new Sprite(bitmap);
        this._controlsSprite.x = (Graphics.width - bitmap.width) / 2;
        this._controlsSprite.y = (Graphics.height - bitmap.height) / 2;
        this.addChild(this._controlsSprite);
    });
};

Scene_Controls.prototype.popScene = function() {
    SceneManager.pop(); // Go back to the previous scene
};
